  ...
  async function handleChangePassword() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPwError(data.error ?? 'Update ADMIN_PASS in Vercel project settings, then redeploy.')
      }
    } catch { setPwError('Update ADMIN_PASS in Vercel project settings, then redeploy.') }
    setPwLoading(false)
  }
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className={`w-full max-w-sm rounded-2xl border p-5 sm:p-6 ${isLight ? 'bg-white border-black/10 text-slate-900' : 'bg-slate-900 border-white/10 text-white'}`}>
            ...
            <p className={`text-xs mb-4 ${isLight ? 'text-slate-400' : 'text-white/40'}`}>
              Your new password is stored server-side as a bcrypt hash.
            </p>
            {pwSuccess ? (
