...
export default function AnalyticsPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    const [qRes, dataRes] = await Promise.all([
      fetch(`/api/admin/questions?${new URLSearchParams({ survey_type: surveyType })}`),
      fetch('/api/admin/data'),
    ])
    const qData = await qRes.json().catch(() => null)
    const allData = await dataRes.json().catch(() => null)
    if (qRes.ok) setQuestions(qData?.questions ?? [])
    if (dataRes.ok) setResponses((allData?.responses ?? []).filter((r: Response) => r.survey_type === surveyType))
    setLoading(false)
  }, [surveyType])
