...
export default function QuestionsPage() {
  ...
  useEffect(() => { loadQuestions() }, [surveyType])
  ...
  async function handleReorder(newOrder: Question[]) {
    ...
    await Promise.all(newOrder.map((q, i) =>
  ...
  async function handleDuplicate(q: Question) {
    ...
    await supabase.from('questions').insert({ ...rest, title: q.title + ' (Copy)', order_index: questions.length })
    loadQuestions()
  }
